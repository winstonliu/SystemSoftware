  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        -- 
          print ("Initializing Thread Manager...\n")
					...
					tmFree = new Condition
					tmFree.Init()

					threadTable[0].Init("T0")
					threadTable[0].status = UNUSED
					freeList.AddToEnd(&threadTable[0])

					threadTable[1].Init("T1")
					threadTable[1].status = UNUSED
					freeList.AddToEnd(&threadTable[1])

					threadTable[2].Init("T2")
					threadTable[2].status = UNUSED
					freeList.AddToEnd(&threadTable[2])

					threadTable[3].Init("T3")
					threadTable[3].status = UNUSED
					freeList.AddToEnd(&threadTable[3])

					threadTable[4].Init("T4")
					threadTable[4].status = UNUSED
					freeList.AddToEnd(&threadTable[4])

					threadTable[5].Init("T5")
					threadTable[5].status = UNUSED
					freeList.AddToEnd(&threadTable[5])

					threadTable[6].Init("T6")
					threadTable[6].status = UNUSED
					freeList.AddToEnd(&threadTable[6])

					threadTable[7].Init("T7")
					threadTable[7].status = UNUSED
					freeList.AddToEnd(&threadTable[7])

					threadTable[8].Init("T8")
					threadTable[8].status = UNUSED
					freeList.AddToEnd(&threadTable[8])

					threadTable[9].Init("T9")
					threadTable[9].status = UNUSED
					freeList.AddToEnd(&threadTable[9])
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- 
					...
					pThread = freeList.Remove()
					(*pThread).status = JUST_CREATED
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- to the FREE list.
        --
					FatalError("Don't call me, baby, anymore.")
					processManagerLock.Lock()
      ...
      method TurnIntoZombie (p: ptr to ProcessControlBlock)	
					var
						curr_PCB: ptr to ProcessControlBlock
						i: int

					processManagerLock.Lock()
					-- Find children
					for i = 0 to MAX_NUMBER_OF_PROCESSES-1
						curr_PCB = &processTable[i]
						if curr_PCB.parentsPid == p.pid && curr_PCB.status == ZOMBIE
							curr_PCB.status = FREE
							freeList.AddToEnd(curr_PCB)
							aProcessBecameFree.Signal(&processManagerLock)
						endIf
					endFor
					-- Find parents
					for i = 0 to MAX_NUMBER_OF_PROCESSES-1
						curr_PCB = &processTable[i]
						if p.parentsPid == curr_PCB.pid && curr_PCB.status == ACTIVE
...
-----------------------------  ProcessFinish  --------------------------
  ...
  function ProcessFinish (exitStatus: int)
    ...
    --
			...
			oldIntStat = SetInterruptsTo (DISABLED)
			endPCB = currentThread.myProcess	
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
					...
					frameManagerLock.Lock()
					numToRet = aPageTable.numberOfPages	

					for i = 0 to numToRet-1
					...
					numberFreeFrames = numberFreeFrames + numToRet
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  function Handle_Sys_Fork () returns int
			var
				myTh: ptr to Thread
				myPCB: ptr to ProcessControlBlock
				oldUserPC, junk, num_parents_pages, i: int

			-- Setup new Thread and PCB objects
			myTh = threadManager.GetANewThread()
			myTh.Init("ChildProcess")
			-- Initialize PCB
			myPCB = processManager.GetANewProcess()
			myPCB.myThread = currentThread 
			myPCB.parentsPid = currentThread.myProcess.pid
			-- Inherit process status
			myTh.status = currentThread.status
			myTh.myProcess = myPCB

			SaveUserRegs(&myTh.userRegs[0])
			...
			for i = 0 to num_parents_pages-1
				MemoryCopy(myPCB.addrSpace.ExtractFrameAddr(i), currentThread.myProcess.addrSpace.ExtractFrameAddr(i), PAGE_SIZE)
				if (currentThread.myProcess.addrSpace.IsWritable(i))
					myPCB.addrSpace.SetWritable(i)
				else
					myPCB.addrSpace.ClearWritable(i)
				endIf 
			endFor

			oldUserPC = GetOldUserPCFromSystemStack()
			myTh.Fork(ResumeChildAfterFork, oldUserPC)
      return myPCB.pid
    endFunction

	function ResumeChildAfterFork(initUserPC: int)
			var
				initUserStackTop, initSystemStackTop, oldIntStat: int
			...
			initSystemStackTop = (&currentThread.systemStack[SYSTEM_STACK_SIZE-1]) asInteger
			initUserStackTop = currentThread.userRegs[14] -- Initial user stack top stored in r15
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  function Handle_Sys_Join (processID: int) returns int
			var
				i: int
				currPCB: ptr to ProcessControlBlock

			for i = 0 to MAX_NUMBER_OF_PROCESSES-1
				currPCB = &processManager.processTable[i]
				if currPCB.status != FREE && currPCB.pid == processID && currPCB.parentsPid == currentThread.myProcess.pid
					return processManager.WaitForZombie(currPCB)
				endIf
			endFor
      return -1
    endFunction
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
			var
				buf: array [MAX_STRING_SIZE] of char	
				oldPtr, i, slot_idx: int
				flag_foundslot: bool
				newFilePtr: ptr to OpenFile
			...
			-- Return if no slot found
			if flag_foundslot == false
				return -1
			endIf

			if StrEqual(buf, "terminal")
				newFilePtr = fileManager.serialTerminalFile
			else 
				newFilePtr = fileManager.Open(&buf)
...
-----------------------------  Handle_Sys_Close  ---------------------------------
  ...
  function Handle_Sys_Close (fileDesc: int)
			...
			if 0 <= fileDesc && fileDesc < MAX_FILES_PER_PROCESS
				if currentThread.myProcess.fileDescriptor[fileDesc] == "terminal"
					currentThread.myProcess.fileDescriptor[fileDesc] = null
				else if currentThread.myProcess.fileDescriptor[fileDesc] != null && 
	...
	behavior SerialDriver
      ----------  SerialDriver . Inti  ----------
			...
			method Init ()
					...
					getBuffer = new array [SERIAL_GET_BUFFER_SIZE] of char 
					getBufferSize = 0
					getBufferNextIn = 0
					getBufferNextOut = 0

					putBuffer = new array [SERIAL_PUT_BUFFER_SIZE] of char 
					putBufferNextIn = 0
					putBufferNextOut = 0

					myTh = threadManager.GetANewThread()
					myTh.Init("SerialMonitorThread")
					serialHandlerThread = *myTh
					myTh.Fork(SerialHandlerFunction, "DEADBEEF" asInteger)

					serialHasBeenInitialized = true
				endMethod

			method SerialHandlerFunction ()
      ...
      ----------  SerialDriver . SerialHandler  ----------
			...
			method SerialHandler ()
					var
						inChar, going_out: char

					while true
						serialNeedsAttention.Wait()

						serialStatusBit = ((*serial_status_word_address) & SERIAL_OUTPUT_READY_BIT >> 1)

						if serialStatusBit == 1
							serialLock.Lock()
							
							if putBufferSize > 0
								going_out = putBuffer[putBufferNextOut]	
