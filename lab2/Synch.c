...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Lock ()
					var oldState: int
					oldState = SetInterruptsTo (DISABLED)
					if self.IsHeldByCurrentThread ()
						FatalError ("Thread already holds lock")
					endIf 

					while heldBy != null-- Spin Lock
						waitList.AddToEnd (currentThread)	
						currentThread.Sleep ()
					endWhile
					...
					oldState = SetInterruptsTo (oldState)
        endMethod
      ...
      method Unlock ()
					...
					if self.IsHeldByCurrentThread ()
						...
						-- Debugging info
						print (heldBy.name)
						print (" holds the lock. ")
						print (currentThread.name)
						print (" attempted unlock.\n")

						FatalError ("Thread does not hold lock")
					endIf
					oldState = SetInterruptsTo (oldState)
        endMethod
